...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cursor.execute("SELECT name, username, photo_path, hashtags FROM users WHERE user_id = ?", (user_id,))
    result = cursor.fetchone()
    conn.close()

    if result:
        name, username, photo_path, hashtags = result
...
async def handle_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Имя сохранено! Теперь загрузите своё фото.")
    return PHOTO
...
async def handle_hashtags(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT OR REPLACE INTO users (user_id, name, username, photo_path, hashtags) VALUES (?, ?, ?, ?, ?)",
        (
...
async def show_profiles(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cursor.execute("SELECT hashtags FROM users WHERE user_id = ?", (user_id,))
    user_hashtags = cursor.fetchone()

    if not user_hashtags:
        await query.message.reply_text("Ваш профиль не найден. Зарегистрируйтесь с помощью команды /start.")
        return

    user_hashtags = set(user_hashtags[0].split(', '))

    cursor.execute("SELECT user_id, name, username, photo_path, hashtags FROM users WHERE user_id != ?", (user_id,))
    ...
    for profile_user_id, name, username, photo_path, hashtags in profiles:
        profile_hashtags = set(hashtags.split(', '))
        if len(user_hashtags.intersection(profile_hashtags)) >= 2:
    ...
    if recommended_profiles:
        ...
        await query.message.reply_text("Подходящих анкет не найдено.")
...
async def show_liked_user_profile(update: Update, context: ContextTypes.DEFAULT_TYPE, liked_user_id: int):
    ...
    cursor.execute("SELECT name, username, photo_path, hashtags FROM users WHERE user_id = ?", (liked_user_id,))
    profile = cursor.fetchone()
    conn.close()

    if profile:
        name, username, photo_path, hashtags = profile
        profile_text = (
            f"<b>Имя:</b> {name}\n"
            f"<b>Хэштеги:</b> {hashtags}"
        )
